<?php
	...
	for(;$numberOfPlayers >= 0; $numberOfPlayers--){
		...
		if($result["player_number"] == $turnOwner){
	...
	if($decksWithCards != 1 or $currentOwner != $winner or (substr($cards[0],0, strlen($cards[0])-1) == substr($cards[1],0, strlen($cards[1])-1) and $decks[0] > 1)){
		$winner = 0;
	}
